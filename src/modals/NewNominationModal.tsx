...
export default function NewNominationModal(props: NewNominationProps) {
  ...
  function isNominationMatch(nomination: INomination): boolean {
    return (
      nominationRequest.movieTitle === nomination.movieTitle &&
      nominationRequest.releaseDate === nomination.releaseDate
    );
  }

  const createNomination = () => {
    api
      .post<IMnmApiResponse<INominationRequest>>(
        '/nomination/create',
        nominationRequest
      )
      .then(
        (res) => {
          if (res.data.status.success && res.data.data !== null) {
            ...
            toast.success(
              `Nomination created for '${nominationRequest.movieTitle}'`
            );
  ...
  const [selectedMovie, setSelectedMovie] = useState<IMovieSearchResult | null>(
    null
  );
    ...
    api
      ...
      .then((res) => {
        if (res.data.data && res.data.status.success) {
          res.data.data.total_results > 0
            ? setMovieOptions(res.data.data.results)
            : toast.error(
                `0 Results for ${searchTitle}, please try another Movie Title`
              );
        }
      })
      .catch((err) => {
  ...
  const updateMovieSelection = async (movie: IMovieSearchResult | null) => {
    ...
    movie?.posterPath
      ? setPreviewPosterPath(
          'https://image.tmdb.org/t/p/w500' + movie.posterPath
        )
      : setPreviewPosterPath('/missingPoster.png');
  };

  async function populateMovieDetails(movieId: number) {
    return api
      .get<IMnmApiResponse<IMovieDetails>>(
        `/tmdb/movie/${movieId}?append_to_response=credits`
      )
      .then((res) => {
        if (res.data.data && res.data.status.success) {
