...
export default function NewWatchPartyModal(props: ModalType) {
  ...
  const [watchPartyRequestState, setWatchPartyRequestState] = useState(
    initialWatchPartyRequest
  );

  const updateWatchDateSelector = (
    selectedWatchDate: dayjs.Dayjs,
    lockDate?: boolean
  ) => {
    const parsedDate = selectedWatchDate
      .toISOString()
    ...
    setWatchPartyRequestState({
      ...
      chosenWatchDate: lockDate
        ? watchPartyRequestState.chosenWatchDate
        : parsedDate,
  ...
  const createWatchParty = (request: IWatchPartyRequest) => {
    api
      .post<IMnmApiResponse<IWatchParty>>('/segment/create', request)
      .then(
        (res) => {
          if (res.data.data) {
            props.setWatchParty(
            ...
            <Box sx={{ textAlign: 'center', marginTop: '10px' }}>
              <div className='input-outer-container '>
                <div className='input-inner-container'>
                  ...
                  <DateSelector
                    handleChangeDate={updateWatchDateSelector}
                    sx={{ marginTop: '8px' }}
                  ></DateSelector>
                  <div className='input-field-text'> Nomination Lock Date</div>
                  <DateSelector
                    handleChangeDate={(date) =>
                      updateWatchDateSelector(date, true)
                    }
                    sx={{ marginTop: '8px' }}
                  />
                </div>
